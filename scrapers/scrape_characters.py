...
DATA_FILENAME_CHARACTERS = 'characters.csv'
DATA_FILENAME_RELATIONS = 'relations.csv'
URL_PARTS = [
...
def getCharacters(url_parts: list[str]) -> list[list[Character], list[Relation]]:
    subpaths: list[str] = []
    ...
    characters_data: list[list[Character], list[Relation]] = [[], []]

    for subpath in subpaths:
        print(f"Scraping info on {subpath}")
        character, relation = getCharacter(subpath)
        characters_data[0].append(character)
        characters_data[1] += relation
        time.sleep(0.5)
...
def getCharacter(subpath: str) -> [Character, list[Relation]]:
...
def getChapterFormatted(part:str|None, chapter: str) -> str:
    """
...
def getCharacterRelations(soup: BeautifulSoup, src_name:str) -> list[Relation]:
    relations:list[Relation] = []

    try:
        relationships_title = soup.find("h2", string="Relationships")
        tag = relationships_title.find_next_sibling()
        genre = ""
        ...
        while tag.name != "h2":
            ...
            if tag.name == "ul":
                for li in tag.find_all("li"):
                    ...
                    if src_name == "DIO":
                        src_name = "Dio Brando"
                    if character_linked == "DIO":
                        character_linked = "Dio Brando"

                    relations.append(Relation(set([src_name, character_linked]), genre))
...
if __name__ == "__main__":
    characters, relations = getCharacters(URL_PARTS)
    dataclasses_to_csv(characters, DATA_PATH, DATA_FILENAME_CHARACTERS)
    dataclasses_to_csv(relations, DATA_PATH, DATA_FILENAME_RELATIONS)